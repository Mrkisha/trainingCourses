...
// get the status so we know if invoice button needs to be hidden
	$stm_status = $db->prepare("SELECT `status` FROM `mod_foxycart_finance_tasks` WHERE orderId = :orderID AND `task` = 2");
	$stm_status->bindParam(":orderID", $_POST['orderId']);
	$stm_status->execute();

	$data_status = $stm_status->fetch(PDO::FETCH_ASSOC);

// get invoice number
	$stm_orders_paymentDate = $db->prepare("SELECT `paymentDate` FROM `mod_foxycart_orders` WHERE orderId = :orderID" );
	$stm_orders_paymentDate->bindParam(":orderID", $_POST['orderId']);
	$stm_orders_paymentDate->execute();

	$data_orders_paymentDate = $stm_orders_paymentDate->fetch(PDO::FETCH_ASSOC);
...
<div class="leftCol">
	<table class="fc_details finance">
			...
			<?php
				foreach($data as $key){
					...
					echo "<td>
							...
							<input type='hidden' name='Id' value='{$_POST['orderId']}'>
							</td>";
				}
		...
		?>
			<tr>
                    <td colspan="4">
                        ...
                        <fieldset>
                            <label><input type="text" id="date"  size="10" name="invoiceDate"></label>
                            <div class="formRight"><input type="button" value="+ Payment" class="blueBtn paymentBtn"></div>
                            <div class="fix"></div>
                        </fieldset>
                        </form>
                    </td>
        ...
        <?php 
       	...
       	?>
       			<tr><td colspan='2'>Payment Date:</td><td colspan='2'><?php echo $data_orders_paymentDate['paymentDate']; ?></td></tr>
		<?php
        	}
        ?>
		</tfoot>
