...
// get the status so we know if invoice button needs to be hidden
	$stm_status = $db->prepare("SELECT `status` FROM `mod_foxycart_finance_tasks` WHERE orderId = :orderID");
...
// get invoice number
	$stm_invoice_num = $db->prepare("SELECT `invoiceNumber` FROM `mod_foxycart_orders` WHERE orderId = :orderID");
	$stm_invoice_num->bindParam(":orderID", $_POST['orderId']);
	$stm_invoice_num->execute();

	$data_invoice_num = $stm_invoice_num->fetch(PDO::FETCH_ASSOC);
...
<div class="leftCol">
	<table class="fc_details finance">
				...
				<tr>
	                <td colspan="4">
	                    ...
	                    <fieldset>
	                        ...
	                        <div class="formRight"><input type="button" value="+ Invoice" class="blueBtn invoiceBtn"></div>
	                        <div class="fix"></div>
	                    </fieldset>
	                    </form>
	                </td>
        ...
        ?>
        		<tr><td colspan='2'>Invoice Number:</td><td colspan='2'><?php echo 	$data_invoice_num['invoiceNumber'] ?></td></tr>        		
		<?php
        	}
        ?>
		</tfoot>
