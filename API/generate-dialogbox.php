			...
			<?php
				foreach($data as $key){
					...
					*/
					...
					if($status == 1){
						echo "<li> <span>Added to Wisenet:</span> <span>".date("d/m/Y H:s", strtotime($date_completed))."</span> </li>";
					}
					echo "</ul></div></td>";
					echo "<td>{$key['productQuantity']}</td>";
					$price_total = $key['productPrice'] * $key['productQuantity'];
					echo "<td><span>\${$price_total}<br />(\${$key['productPrice']} each)</span>";

					// if status of product = 1 then we dont show this button
					if($status == 0){
						echo "<input type=\"button\" value=\"+ Wisenet\" class=\"blueBtn\">
