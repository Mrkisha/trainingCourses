...
<div class="wrapper"> 
	...
	<div class="leftNav">
		<ul id="menu">
			<li class="tables"><a href="tables.html" title="" class="active"><span>Public Courses</span><span class="numberLeft">6</span></a></li>
	...
	<div class="content">
		...
		<div class="stats">
			<ul>
				...
				<li class="last"><a href="#" class="count red" title="">2</a><span>overdue tasks</span></li>
			</ul>
			<div class="fix"></div>
		</div>
			...
			<?php
                ...
                /*$stm = $db->prepare("SELECT DISTINCT
                                 ...
                                 FROM
                                     ...
                                     INNER JOIN `mod_foxycart_customers` 

                                         ON (`mod_foxycart_orders`.`orderId` = `mod_foxycart_customers`.`orderId`)

                                     INNER JOIN `mod_foxycart_training_tasks` 

                                         ON (`mod_foxycart_orders`.`orderId` = `mod_foxycart_training_tasks`.`orderId`)

                                 WHERE (`mod_foxycart_training_tasks`.`task` =1

                                     AND `mod_foxycart_training_tasks`.`status` =0)");
...
*/
			...
			<table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
                ...
                while($row = $data){
                    ...
                    echo "<td>{$row[0]['orderDate']}</td>";

                    echo "<td>{$row[0]['orderId']}</td>";

                    echo "<td>{$row[0]['orderDate']}</td>";

                    echo "<td>{$row[0]['orderTotal']}</td>";

                    echo "<td>{$row[0]['customerFirstName']} {$row[0]['customerLastName']}</td>";
